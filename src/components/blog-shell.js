...
export default function BlogShell({
  ...
  // Page scroll — drives the masthead title parallax only.
  const pageRef = useRef(null);
  const { scrollYProgress: pageProgress } = useScroll({
    target: pageRef,
    offset: ["start start", "end end"],
  });
  const pageSmooth = useSpring(pageProgress, {
    stiffness: 80,
    damping: 30,
  });
  const titleY = useTransform(pageSmooth, [0, 0.15], ["0%", "-25%"]);
  const titleOpacity = useTransform(pageSmooth, [0, 0.12], [1, 0.25]);

  // Article scroll — drives the vine drawing. Starts when the article
  // reaches the top of the viewport (i.e. after the masthead has
  // parallaxed up and out) and completes as the article exits.
  ...
  return (
    ...
    >
      ...
      {/* Masthead — parallax + radial gradient */}
      <Box
        component="header"
        sx={{
          position: "relative",
